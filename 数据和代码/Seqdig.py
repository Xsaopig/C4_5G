...
if __name__ == '__main__':
    ...
    ## 生成所有时间序列
    Seqs = dp.GenSeqs_3d(data,600)
    print('共有'+str(len(Seqs))+'个序列')
    

    # 对序列进行字符串化,去除序列中每个项目集的连续的重复项
    for i in range(len(Seqs)):
        for j in range(len(Seqs[i])):
            # Seqs[i][j] = list(map(lambda x,y:str(x)+'-'+str(y),[locToIndexOflocs[x[5]] for x in Seqs[i][j]],[x[1] for x in Seqs[i][j]])) # 序列中项集中的项为 设备定位信息+告警代码
            # Seqs[i][j] = list(map(lambda x:str(x),[x[1] for x in Seqs[i][j]]))  # 序列中项集中的项为告警代码
            # Seqs[i][j] = list(map(lambda x:x,[locToIndexOflocs[x[5]] for x in Seqs[i][j]]))  # 序列中项集中的项为设备定位信息
            tmp = list(map(lambda x:x,[locToIndexOflocs[x[5]] for x in Seqs[i][j]]))
            Seqs[i][j] = tmp
            pass
        Seqs[i] = dp.Seq_compression(Seqs[i]) #去除序列中每个项目集的连续的重复项
    # 按序列长度排个序
    ...
    # ## 频繁序列挖掘 SPAM
    # freqSeqs = Mining_FreSeqPatterns(Seqs,minsp=30.0/len(Seqs),minpatternlen=2,maxpatternlen=10,maxgap=10)
    ...
    ## 序列规则挖掘 RuleGrowth
    Seqrules = Mining_Seqrules(Seqs,minsp=30.0/len(Seqs),minconf=0.3,max_antecedent_len=10,max_consequent_len=10)
    # 保存挖掘出的序列规则
